...
import { flightOrderDetail, ApiError } from "@/lib/order-api";
...
export async function POST(req: NextRequest) {
  ...
  if (!body.system_no && !body.out_trade_no) {
    return NextResponse.json(
      { code: 400, message: "需要 system_no 或 out_trade_no", data: null },
      { status: 400 }
    );
  }
  try {
    const data = await flightOrderDetail(body.system_no, body.out_trade_no);
